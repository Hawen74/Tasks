...
// SIGNUP
// =====================================================
app.post("/signup", signupLimiter, validateBody(inputSchema), async (req, res) => {
    try {
        ...
        return res.status(201).json(sanitizeUser(newUser.rows[0]));
    }
    catch (error) {
        console.error(error);
        return res.status(500).json({
...
// LOGIN
// =====================================================
app.post("/login", loginLimiter, validateBody(inputSchema), async (req, res) => {
    try {
        const { email, password, } = req.body;
        const result = await pool.query(`
                SELECT
...
// REFRESH
// =====================================================
app.post("/refresh", validateBody(refreshTokenSchema), (req, res) => {
...
// LOGOUT
// =====================================================
app.post("/logout", validateBody(refreshTokenSchema), (req, res) => {
    const { refreshToken, } = req.body;
    if (!refreshTokenSet.has(refreshToken)) {
        return res.status(401).json({
            error: "Invalid or expired refresh token",
...
// POST TASK
// =====================================================
app.post("/tasks", requireAuth, validateBody(dataSchema), async (req, res) => {
    try {
...
// PUT TASK
// =====================================================
app.put("/tasks/:id", requireAuth, validateBody(dataSchema), async (req, res) => {
    try {
